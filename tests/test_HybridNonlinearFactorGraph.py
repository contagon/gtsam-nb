...
class TestHybridGaussianFactorGraph(GtsamTestCase):
    ...
    def test_nonlinear_hybrid(self):
        ...
        nlfg.push_back(
            BetweenFactorPoint3(
                1, 2, Point3(1, 2, 3), noiseModel.Diagonal.Variances([1, 1, 1])
            )
        )
        nlfg.push_back(
            PriorFactorPoint3(
                2, Point3(1, 2, 3), noiseModel.Diagonal.Variances([0.5, 0.5, 0.5])
            )
        )
        nlfg.push_back(
            gtsam.MixtureFactor(
                ...
                dk,
                [
                    PriorFactorPoint3(1, Point3(0, 0, 0), noiseModel.Unit.Create(3)),
                    PriorFactorPoint3(1, Point3(1, 2, 1), noiseModel.Unit.Create(3)),
                ],
            )
        )
