...
Optimizer = (
    ...
    | type[gtsam.LevenbergMarquardtOptimizer]
    | type[gtsam.DoglegOptimizer]
    | type[gtsam.GncGaussNewtonOptimizer]
    | type[gtsam.GncLMOptimizer]
    | type[gtsam.GncLMOptimizer]
)
OPTIMIZER_PARAMS_MAP: dict[
    Optimizer | type[gtsam.NonlinearOptimizer], type[gtsam.NonlinearOptimizerParams]
] = {
...
def optimize_using(
    OptimizerClass: type[gtsam.NonlinearOptimizer],
    hook: Callable[[gtsam.NonlinearOptimizer, float], None],
    *args: Any,
) -> gtsam.Values:
...
def optimize(
    ...
    check_convergence: Callable[[gtsam.NonlinearOptimizer, float, float], bool],
    hook: Callable[[gtsam.NonlinearOptimizer, float], None],
):
