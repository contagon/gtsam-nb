...
class Data:
    ...
    class NoiseModels:
        posePrior: gtsam.noiseModel.Diagonal
        odometry: gtsam.noiseModel.Diagonal
        pointPrior: gtsam.noiseModel.Isotropic
...
def generate_data(options: Options) -> Tuple[Data, GroundTruth]:
    ...
    for i in range(options.nrCameras):
        ...
        truth.cameras[i] = PinholeCameraCal3_S2.Lookat(
            t, Point3(0, 0, 0), Point3(0, 0, 1), truth.K
        )
        # Create measurements
    ...
    for i in range(1, options.nrCameras):
        data.odometry[i] = truth.cameras[i - 1].pose().between(truth.cameras[i].pose())
