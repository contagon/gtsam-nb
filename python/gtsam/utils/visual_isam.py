...
def initialize(data: Data, truth: GroundTruth, options: Options):
    # Initialize iSAM
    ...
    newFactors.add(
        gtsam.BetweenFactorPose3(
            symbol("x", 0), symbol("x", 1), data.odometry[1], data.noiseModels.odometry
        )
    )

    # Update ISAM
    if options.batchInitialization:  # Do a full optimize for first two poses
        batchOptimizer = gtsam.LevenbergMarquardtOptimizer(newFactors, initialEstimates)
...
def step(
    data: Data, isam: ISAM2, result: Values, truth: GroundTruth, currPoseIndex: int
):
    """
    ...
    initialEstimates.insert(symbol("x", currPoseIndex), prevPose.compose(odometry))
